...
> 이 문서는 `python3 -m src.main` 실행 시 자동 생성됩니다. (생성 시각: 2026-07-21 20:06)
...
| 항목 | Pandas | Polars |
| --- | --- | --- |
| 로딩 시간 (3회 평균) | 1.2853초 | 0.0710초 |
| 형태 (행 × 열) | 65,437 × 114 | 65,437 × 114 |
| 관심 컬럼 결측치 | 126,701 | 126,701 |

→ **Polars가 약 18.1배 빠릅니다.** 형태 일치: ✅ · 결측치 집계 일치: ✅
...
| 모델 | `RidgeCV(alpha=3.162)` | 타깃: `ConvertedCompYearly` |
...
| 지표 | 값 | 의미 |
| --- | --- | --- |
| R² Score | 0.5563 | 모델이 설명하는 연봉 분산의 비율 |
| RMSE | $42,430 | 예측 오차의 제곱평균제곱근 |
| MAE | $28,793 | 예측 오차의 절댓값 평균 |
...
| 요인 | 계수 |
| --- | --- |
| Country_United States of America | +88,961 |
| Country_Switzerland | +67,252 |
| Country_Singapore | +55,780 |
| Country_Israel | +54,815 |
| Age_65 years or older | -43,588 |
| Country_Australia | +41,731 |
| Country_Canada | +41,473 |
| Country_United Kingdom of Great Britain and Northern Ireland | +41,469 |
| Country_Denmark | +41,082 |
| DevType_Senior Executive (C-Suite, VP, etc.) | +36,658 |

#### AISent 순효과 — "다 통제해도 역설이 남는가"

| AI 태도 | 계수 |
| --- | --- |
| Very unfavorable | +1,954 |
| Very favorable | +1,620 |
| Favorable | -172 |
| Indifferent | -191 |
| Unfavorable | -1,317 |
| Unsure | -1,894 |

→ 학력·조직·근무형태·연령·경력·국가·직군을 **모두 통제한 뒤에도** Very unfavorable 계수(+1,954)가 Very favorable(+1,620)보다 높습니다. **역설은 교란변수로 설명되지 않고 잔존합니다.**
...
| --- | --- | --- | --- |
| 로딩(read_csv) | 1.2853 | 0.0710 | 18.09x |
| 필터(notna) | 0.0091 | 0.0089 | 1.02x |
| 집계(groupby mean) | 0.0025 | 0.0074 | 0.34x |
