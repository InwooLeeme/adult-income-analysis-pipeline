...
def run_statistics(df: pd.DataFrame, reporter: Reporter) -> StatsResult:
    ...
    experience_corr, experience_corr_p = stats.pearsonr(df["YearsCodePro"], df[TARGET_COLUMN])
    ...
    reporter.log(
        f"- 상관계수 r = {result.experience_corr:.3f}, {format_p_expression(result.experience_corr_p)}"
    )
    reporter.log(f"-> {describe_correlation(result.experience_corr)}")

    reporter.log("\n[T-test] AI 우호 그룹 vs 그 외 그룹의 평균 연봉 차이")
    reporter.log(f"- 우호 그룹: {result.favorable_size:,}명, 평균 ${result.favorable_mean:,.0f}")
    reporter.log(f"- 그 외 그룹: {result.others_size:,}명, 평균 ${result.others_mean:,.0f}")
    reporter.log(f"- t-statistic = {result.t_statistic:.3f}, {format_p_expression(result.p_value)}")
    reporter.log(f"-> [해석] {interpret_p_value(result.p_value)}")
    reporter.log(
        f"- 평균 차이 ${result.mean_gap:,.0f}, 95% CI [${result.ci_low:,.0f}, ${result.ci_high:,.0f}]"
    )
    reporter.log(f"- Cohen's d = {result.cohens_d:.3f} -> {interpret_cohens_d(result.cohens_d)}")
