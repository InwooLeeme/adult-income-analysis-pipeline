...
def _overview_section(data_path: Path, prep: PreprocessResult, stats_result: StatsResult) -> str:
    """1장 — 분석 주제와 가설, 데이터 개요."""
    return "\n".join(
        [
            "## 1. 분석 개요",
...
def _data_preparation_section(load: LoadResult, prep: PreprocessResult) -> str:
    ...
    return "\n".join(
        [
            ...
            "",
            f"→ **{prep.raw_rows:,}건 → {prep.clean_rows:,}건** ({prep.retention_rate:.1%} 잔존)",
            "",
        ]
    )
        ...
        [
            ...
            markdown_table(
                ...
                [
                    ...
                    [
                        "모델",
                        f"`RidgeCV(alpha={model.selected_alpha:.4g})`",
                        f"타깃: `{TARGET_COLUMN}`",
                    ],
                ],
            ),
...
def _multivariate_section(result: MultivariateResult) -> str:
    ...
    num_prefix, cat_prefix = f"{NUM_TRANSFORMER_NAME}__", f"{CAT_TRANSFORMER_NAME}__"
    top_rows = [
        [name.replace(num_prefix, "").replace(cat_prefix, ""), f"{val:+,.0f}"]
        for name, val in result.top_coefficients
    ]
    ai_rows = [
        [name.replace(f"{cat_prefix}AISent_", ""), f"{val:+,.0f}"] for name, val in result.ai_coefficients
    ]
    paradox_note = (
