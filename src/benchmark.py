...
기능:
    - run_benchmark(): 로딩·필터·집계 3개 작업을 각각 여러 번 실행해 두 라이브러리의
      평균 처리 시간을 재고, 이후 단계가 재사용할 DataFrame과 함께 반환한다.
    - _save_benchmark_chart(): 작업별 처리 시간을 비교하는 막대차트를 저장한다.

1단계 load_and_compare()가 "결과 정합성"을 보는 것과 목적이 달라(순수 속도) 별도 단계로 두되,
로딩에서 만든 DataFrame과 소요 시간은 그대로 넘겨 파일을 두 번 읽지 않도록 한다.
"""
...
def _time_and_capture(func, runs: int) -> tuple[float, object]:
    """func()를 runs회 실행해 평균 소요 시간과 마지막 실행 결과를 함께 반환한다.

    timeit은 반환값을 버리므로, 로딩 결과(DataFrame)를 재사용하려면 직접 측정해야 한다.
    """
    total = 0.0
    result = None
    for _ in range(runs):
...
def run_benchmark(
...
) -> tuple[BenchmarkResult, pd.DataFrame, pl.DataFrame]:
    """동일 작업(로딩·필터·집계)을 두 라이브러리로 각 runs회 실행해 평균 시간을 잰다.

    로딩 단계에서 만든 DataFrame(df_pd, df_pl)도 함께 반환해, 이후 load_and_compare()가
    같은 파일을 다시 읽지 않고 정합성 비교에 재사용할 수 있게 한다.
    """
    reporter.section("0. Pandas vs Polars 성능 벤치마크")

    # 로딩: 파일 → DataFrame (I/O 포함). 마지막 실행 결과를 그대로 재사용한다.
    pd_load, df_pd = _time_and_capture(lambda: pd.read_csv(data_path, low_memory=False), runs)
    pl_load, df_pl = _time_and_capture(
        lambda: pl.read_csv(str(data_path), null_values=POLARS_NULL_VALUES, ignore_errors=True),
        runs,
    )

    # 필터·집계는 이미 메모리에 올라온 프레임 대상 (I/O 제외, 순수 연산만 비교)
    pd_filter = timeit.timeit(lambda: df_pd[df_pd[TARGET_COLUMN].notna()], number=runs) / runs
    pl_filter = timeit.timeit(lambda: df_pl.filter(pl.col(TARGET_COLUMN).is_not_null()), number=runs) / runs

    pd_group = timeit.timeit(lambda: df_pd.groupby("Country")[TARGET_COLUMN].mean(), number=runs) / runs
    pl_group = (
        timeit.timeit(
    ...
    for task, pd_sec, pl_sec in result.rows:
        ...
        reporter.log(f"- {task}: Pandas {pd_sec:.4f} / Polars {pl_sec:.4f} (Polars {ratio:.2f}배)")

    _save_benchmark_chart(result, reporter)
    return result, df_pd, df_pl
