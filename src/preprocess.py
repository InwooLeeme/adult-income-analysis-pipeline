...
def preprocess(df: pd.DataFrame, reporter: Reporter) -> tuple[pd.DataFrame, PreprocessResult]:
    ...
    reporter.log("\n✅ [처리 결과]")
    reporter.log(f"- 결측치·중복 제거 및 이상치(상위 1%, ${result.outlier_threshold:,.0f} 초과) 절단")
    reporter.log(f"- {result.raw_rows:,}건 -> {result.clean_rows:,}건 ({result.retention_rate:.1%} 잔존)")
