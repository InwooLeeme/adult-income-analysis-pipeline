...
def interpret_p_value(p_value: float, alpha: float = ALPHA) -> str:
    ...
    verdict = "기각 — 통계적으로 유의미함" if p_value < alpha else "채택 — 통계적으로 유의미하지 않음"
...
def conclude_ttest(result: StatsResult) -> str:
    ...
    winner = "AI에 우호적인" if result.t_statistic > 0 else "AI에 비우호적이거나 무관심한"
    return (
...
def frame_to_markdown(frame: pd.DataFrame, index_header: str, number_format: str = ",.2f") -> str:
...
def series_to_markdown(
    ...
    rows = [[str(index), _format_cell(value, number_format)] for index, value in series.items()]
