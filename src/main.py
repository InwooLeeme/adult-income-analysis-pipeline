...
실행:
    python3 -m src.main
    python3 -m src.main --data-path data/results.csv
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="분석할 설문 CSV 경로 (기본값: data/results.csv)",
    )
...
def main() -> None:
    ...
    reporter = Reporter()
    benchmark_result, df_pd, df_pl = run_benchmark(args.data_path, reporter)
    raw_df, load_result = load_and_compare(df_pd, df_pl, benchmark_result.load, reporter)
