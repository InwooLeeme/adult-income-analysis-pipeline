...
def run_deep_dive(df: pd.DataFrame, reporter: Reporter) -> str:
    ...
    skeptics_earn_more = median_by_sentiment.get("Very unfavorable", 0) > median_by_sentiment.get(
        "Very favorable", 0
    )
    ...
    median_by_experience = (
        df.groupby(["ExperienceGroup", "AISent"], observed=False)[TARGET_COLUMN].median().unstack()
    )
    comparison_rows = []
    skeptic_wins = 0
    for group in experience_labels[:-1]:  # 25년 이상은 표본이 적어 제외
        favorable_median = median_by_experience.loc[group].get("Very favorable", float("nan"))
        unfavorable_median = median_by_experience.loc[group].get("Very unfavorable", float("nan"))
