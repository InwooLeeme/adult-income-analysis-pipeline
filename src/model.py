...
from .config import (
    ...
    NUM_TRANSFORMER_NAME,
    TARGET_COLUMN,
)
from .reporting import Reporter
from .results import ModelResult, MultivariateResult

# 교차검증으로 탐색할 Ridge 정규화 강도 후보. 0.01~1000 범위를 로그 간격으로 훑는다.
RIDGE_ALPHAS = np.logspace(-2, 3, 13)
...
def train_model(df: pd.DataFrame, reporter: Reporter) -> tuple[Pipeline, ModelResult]:
    ...
    preprocessor = ColumnTransformer(
        transformers=[
            (NUM_TRANSFORMER_NAME, StandardScaler(), FEATURE_NUMERIC_COLUMNS),
            (
                CAT_TRANSFORMER_NAME,
    ...
    # alpha는 RidgeCV가 교차검증으로 고른다(고정값 대신).
    pipeline = Pipeline(
        steps=[
    ...
    result = ModelResult(
        ...
        selected_alpha=float(pipeline.named_steps["regressor"].alpha_),
    )
    ...
    reporter.log(f"- RidgeCV 선택 alpha: {result.selected_alpha:.4g}")
...
def run_multivariate(df: pd.DataFrame, baseline_r2: float, reporter: Reporter) -> MultivariateResult:
    ...
    preprocessor = ColumnTransformer(
        transformers=[
            (NUM_TRANSFORMER_NAME, StandardScaler(), num_features),
            (
                CAT_TRANSFORMER_NAME,
                OneHotEncoder(handle_unknown="ignore"),
                cat_features,
    ...
    ai_sent_prefix = f"{CAT_TRANSFORMER_NAME}__AISent"
    ai_coefs = coef_s[[n for n in coef_s.index if n.startswith(ai_sent_prefix)]].sort_values(ascending=False)
    vf = float(coef_s.get(f"{ai_sent_prefix}_Very favorable", 0.0))
    vu = float(coef_s.get(f"{ai_sent_prefix}_Very unfavorable", 0.0))
    ...
    reporter.log(f" - AISent 순효과: Very favorable({vf:+,.0f}) vs Very unfavorable({vu:+,.0f})")
    reporter.log(f" - 역설 지속 여부(통제 후에도 회의론자 계수가 높은가): {result.paradox_persists}")

    # 기본 모델보다 설명력이 높은 확장 모델도 별도 파일로 저장한다.
    multivariate_model_path = MODEL_DIR / MULTIVARIATE_MODEL_NAME
    joblib.dump(pipeline, multivariate_model_path)
    reporter.log(f"✅ 다변량 모델 저장(joblib): {multivariate_model_path.relative_to(BASE_DIR)}")
