...
단순 시각화가 아니라 가설 설정, 전처리, EDA, 통계 검정, 효과크기 해석, RidgeCV 회귀 모델링, 교란변수 통제, 리포트 자동 생성까지 하나의 재현 가능한 흐름으로 구성했습니다.
...
- `tests/`: 전처리, 통계, 모델링, 산출물 생성, 보고서 렌더링 검증
...
| 테스트 | 주요 파이프라인 단계와 보고서 산출물을 pytest로 검증 |
